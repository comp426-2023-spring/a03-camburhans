...
export function rpsls(playerShot) {
    if (playerShot === undefined) {
        return {"player": randomItem(RPSLS)};
    }

    if (!(playerShot.toLowerCase() in RPSLS)) {
  ...
  - Rock CRUSHES Scissors`)
    ...
    let opponentShot = randomItem(RPSLS);
    let result = RPSLS[playerShot.toLowerCase()].includes(opponentShot.toLowerCase()) ? "lose" : "win";
